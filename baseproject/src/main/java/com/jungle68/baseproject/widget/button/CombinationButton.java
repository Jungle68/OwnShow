package com.jungle68.baseproject.widget.button;
...
public class CombinationButton extends FrameLayout {
    ...
    public CombinationButton(Context context, AttributeSet attrs) {
        ...
        int leftTextLeftPadding = array.getDimensionPixelOffset(R.styleable.combinationBtn_leftTextLeftPadding, getContext().getResources().getDimensionPixelOffset(R.dimen.spacing_mid));
        boolean lineIsAlignedMessage = array.getBoolean(R.styleable.combinationBtn_lineIsAlignedMessage, false);
        array.recycle();
        ...
        mCombinedButtonLeftText.setPaddingRelative(leftTextLeftPadding, 0, 0, 0);
        if (leftImage == null) {
        ...
        if (showLine) {
            ...
            if (lineIsAlignedMessage) {
                dividerLeftMargin = (leftImage != null ? (getContext().getResources().getDimensionPixelOffset(R.dimen.spacing_mid)
                        + getContext().getResources().getDimensionPixelOffset(R.dimen.icon_for_user_center)) : 0)
                        + leftTextLeftPadding;
            }
    ...
    public String getRightText() {
        return mCombinedButtonRightText.getText().toString().trim();
    }
}
